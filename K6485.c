...
static void k6485_Operate (int menubar, int menuItem, void *callbackData, int panel)
{
    ...
    SetMenuBarAttribute (m, K6485MENUS_MEASURE_DMM, ATTR_CALLBACK_DATA, dmm->chan[0]->acqchan);
...
static void k6485_UpdateReadings (int panel, void *dev)
{
    ...
    if ((!util_TakingData() || !dmm->chan[chan]->acqchan->acquire)) {
        ...
        dmm->chan[chan]->overLimit =  status & K6485_MSE_ROF;
    }

    SetCtrlVal (panel, K6485_OVER, dmm->chan[chan]->overLimit);
    SetCtrlVal (panel, K6485_MEASURE, dmm->chan[chan]->acqchan->reading);
}
    ...
    switch (control) {
        ...
        case K6485_FILTER_READINGS:
            if (event == EVENT_COMMIT) {
                ...
                Fmt (cmd, "%s:average:count %i", k6485_MeasFunction(dev), dmm->chan[chan]->filter.readings);
                k6485_Out (dev, cmd);
            }
            break;
        case K6485_FILTER_ENABLE:
            if (event == EVENT_COMMIT) {
                ...
                Fmt (cmd, "%s:average:state %i", k6485_MeasFunction(dev), dmm->chan[chan]->filter.enable);
                k6485_Out (dev, cmd);
                SetInputMode (panel, K6485_FILTER_READINGS, dmm->chan[chan]->filter.enable);
                SetInputMode (panel, K6485_FILTER_TYPE, dmm->chan[chan]->filter.enable);
        ...
        case K6485_RANGE_AUTO:
            if (event == EVENT_COMMIT) {
                ...
                if (autorange) dmm->chan[chan]->range = 0.0;
                    ...
                    dmm->chan[chan]->range = k6485_GetRange (dev);
                    SetCtrlVal (panel, K6485_RANGE, dmm->chan[chan]->range);
                }
            }
            break;
        case K6485_RANGE:
            if (event == EVENT_COMMIT) {
                ...
                Fmt (cmd, "%s:range:upper %f[p2]", k6485_MeasFunction (dev), dmm->chan[chan]->range);
                k6485_Out (dev, cmd);
            }
            break;
        case K6485_RATE:
            if (event == EVENT_COMMIT) {
                ...
                Fmt (cmd, "%s:nplc %f[p2]", k6485_MeasFunction(dev), dmm->chan[chan]->rate*0.06);
                k6485_Out (dev, cmd);
            }
            break;
        case K6485_DCV:
...
static double k6485_GetMeasurement (int chan, gpibioPtr dev)
{
    ...
    if (chan != k6485_GetChannel (dev)) {
        ...
        if (dmm->chan[chan]->range == 0.0) {
            ...
            k6485_Out (dev, cmd);
            Fmt (cmd, "%s:range:upper %f", k6485_MeasFunction (dev), dmm->chan[chan]->range);
            k6485_Out (dev, cmd);
        }
        k6485_CheckforProblems (dev);
...
static void k6485_UpdateControls (int panel, gpibioPtr dev)
{
    ...
    k6485_Out (dev, "initiate:continuous on");
    k6485_CheckforProblems (dev);
    ...
    dmm->chan[chan]->rate = k6485_GetRate (dev);
    SetCtrlVal (panel, K6485_RATE, dmm->chan[chan]->rate);

    dmm->chan[chan]->filter.enable = k6485_GetFilterStatus (dev);
    SetCtrlVal (panel, K6485_FILTER_ENABLE, dmm->chan[chan]->filter.enable);
    SetInputMode (panel, K6485_FILTER_READINGS, dmm->chan[chan]->filter.enable);
    SetInputMode (panel, K6485_FILTER_TYPE, dmm->chan[chan]->filter.enable);

    if (dmm->chan[chan]->filter.enable) {
        dmm->chan[chan]->filter.moving = k6485_GetMovingFilterType (dev);
        SetCtrlVal (panel, K6485_FILTER_TYPE, dmm->chan[chan]->filter.moving);

        dmm->chan[chan]->filter.readings = k6485_GetFilterReadings (dev);
        SetCtrlVal (panel, K6485_FILTER_READINGS, dmm->chan[chan]->filter.readings);
    }
...
static int k6485_GetFunction (gpibioPtr dev)
{
    ...
    else if (CompareBytes (rsp, 1, "CURR:DC", 0, 7, 0) == 0) dmm->func = K6485_DCI;
...
static char *k6485_MeasFunction (gpibioPtr dev)
{
    ...
    switch (dmm->func) {
        ...
        case K6485_DCI: Fmt (msg, "current:dc"); break;
