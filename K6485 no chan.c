...
static void k6485_Operate (int menubar, int menuItem, void *callbackData, int panel)
{
   ...
   /*SetMenuBarAttribute (m, K6485MENUS_MEASURE_DMM, ATTR_CALLBACK_DATA, dmm->acqchan);
...
static void k6485_UpdateReadings (int panel, void *dev)
{
    ...
    if ((!util_TakingData() || !dmm->acqchan->acquire)) {
        k6485_Out (dev, "fetch?");
        k6485_In (dev, rsp);
        Scan (rsp, "%s>%f", &dmm->acqchan->reading);
        dmm->overLimit =  status & K6485_MSE_ROF;
    }

    SetCtrlVal (panel, K6485_OVER, dmm->overLimit);
    SetCtrlVal (panel, K6485_MEASURE, dmm->acqchan->reading);
}
...
static void *k6485_Create (gpibioPtr dev)
{
   ...
   /* for (i = 0; i < 11; i++) {
       ...
       // dmm->chan= malloc (sizeof(k6485chanType));
            ...
            dmm->filter.readings = 10;
            dmm->filter.moving = TRUE;
            dmm->range = 0;
        
    

      dmm->acqchan = acqchan_Create ("K6485 Picoammeter", dev, k6485_GetReadings);
...
static void k6485_LoadChannel (int chan, gpibioPtr dev)
{
    ...
    if (dev) {
        dmm = dev->device;
        ScanFile (fileHandle.analysis, "%s>Rate  : %f%s[dw1]", &dmm->rate);
        ScanFile (fileHandle.analysis, "%s>Range : %f%s[dw1]", &dmm->range);
        ScanFile (fileHandle.analysis, "%s>Filter: %i, %i, %i%s[dw1]",
              &dmm->filter.enable, &dmm->filter.moving,
              &dmm->filter.readings);
        acqchan_Load (dev, dmm->acqchan);
    } else {
        ReadLine (fileHandle.analysis, info, 255);
        ReadLine (fileHandle.analysis, info, 255);
        ReadLine (fileHandle.analysis, info, 255);
...
/*static void k6485_SaveChannel (k6485chanPtr chan)
{
    ...
    FmtFile (fileHandle.analysis, "Filter: %i, %i, %i\n",
             chan->filter.enable, chan->filter.moving, filter.readings);
    ...
    switch (control) {
        case K6485_FILTER_TYPE:
            if (event == EVENT_COMMIT) {
                ...
                GetCtrlVal (panel, control, &dmm->filter.moving);
                if (dmm->filter.moving)
        ...
        case K6485_FILTER_READINGS:
            if (event == EVENT_COMMIT) {
                chan = k6485_GetChannel (dev);
                GetCtrlVal (panel, control, &dmm->filter.readings);
                Fmt (cmd, "%s:average:count %i", k6485_MeasFunction(dev), dmm->filter.readings);
                k6485_Out (dev, cmd);
            }
            break;
        case K6485_FILTER_ENABLE:
            if (event == EVENT_COMMIT) {
                chan = k6485_GetChannel (dev);
                GetCtrlVal (panel, control, &dmm->filter.enable);
                Fmt (cmd, "%s:average:state %i", k6485_MeasFunction(dev), dmm->filter.enable);
                k6485_Out (dev, cmd);
                SetInputMode (panel, K6485_FILTER_READINGS, dmm->filter.enable);
                SetInputMode (panel, K6485_FILTER_TYPE, dmm->filter.enable);
                if (dmm->filter.enable) {
                    Fmt (cmd, "%s:average:count %i", k6485_MeasFunction(dev), dmm->filter.readings);
                    k6485_Out (dev, cmd);
                    if (dmm->filter.moving)
                        Fmt (cmd, "%s:average:tcontrol mov", k6485_MeasFunction(dev));
        ...
        case K6485_RANGE_AUTO:
            if (event == EVENT_COMMIT) {
                ...
                if (autorange) dmm->range = 0.0;
                else {
                    dmm->range = k6485_GetRange (dev);
                    SetCtrlVal (panel, K6485_RANGE, dmm->range);
                }
            }
            break;
        case K6485_RANGE:
            if (event == EVENT_COMMIT) {
                chan = k6485_GetChannel (dev);
                GetCtrlVal (panel, control, &dmm->range);
                Fmt (cmd, "%s:range:upper %f[p2]", k6485_MeasFunction (dev), dmm->range);
                k6485_Out (dev, cmd);
            }
            break;
        case K6485_RATE:
            if (event == EVENT_COMMIT) {
                chan = k6485_GetChannel (dev);
                GetCtrlVal (panel, control, &dmm->rate);
                Fmt (cmd, "%s:nplc %f[p2]", k6485_MeasFunction(dev), dmm->rate*0.06);
                k6485_Out (dev, cmd);
            }
            break;
        //case K6485_DCV:
...
static double k6485_GetMeasurement (int chan, gpibioPtr dev)
{
    ...
    dmm->acqchan->reading = reading;
    dmm->acqchan->newreading = TRUE;
    return reading;
}
...
static void k6485_UpdateControls (int panel, gpibioPtr dev)
{
   ...
   // SetCtrlAttribute (panel, k6485_GetFunction (dev), ATTR_CMD_BUTTON_COLOR, VAL_RED);
     dmm->func = K6485_DCI;
    dmm->rate = k6485_GetRate (dev);
    SetCtrlVal (panel, K6485_RATE, dmm->rate);

    dmm->filter.enable = k6485_GetFilterStatus (dev);
    SetCtrlVal (panel, K6485_FILTER_ENABLE, dmm->filter.enable);
    SetInputMode (panel, K6485_FILTER_READINGS, dmm->filter.enable);
    SetInputMode (panel, K6485_FILTER_TYPE, dmm->filter.enable);

    if (dmm->filter.enable) {
        dmm->filter.moving = k6485_GetMovingFilterType (dev);
        SetCtrlVal (panel, K6485_FILTER_TYPE, dmm->filter.moving);

        dmm->filter.readings = k6485_GetFilterReadings (dev);
        SetCtrlVal (panel, K6485_FILTER_READINGS, dmm->filter.readings);
    }
