...
#define TRUE 1
#define FALSE 0
#define DAS6036_ID "PCI-DAS6036"

typedef struct{
    portPtr Achannels[10], Dchannels[1];
    int panel, menu;
}das6036Type;
typedef das6036Type *das6036Ptr;
/***********************************************************************************/
int  das6036_PanelCallback          (int panel, int event, void *callbackData, int eventData1, int eventData2);
int  das6036_ControlCallback        (int panel, int control, int event, void *callbackData, int eventData1, int eventData2);
void das6036_MenuCallback           (int menbar, int menuItem, void *callbackData, int panel);
int  das6036_MeasureControlCallback (int panel, int control, int event, void *callbackData, int eventData1, int eventData2);
int  das6036_IndexFromRange         (int val);

void das6036_UpdateMeasurePanel(portPtr port);

void das6036_Create     (MCCdevPtr dev);
void das6036_Remove         (void* ptr);
int  das6036_InitIO         (MCCdevPtr dev);
void das6036Operate         (int menubar, int menuItem, void *callbackData, int panel);
void das6036_UpdateReadings (int panel, void *ptr);
void das6036_Save           (MCCdevPtr dev);
void das6036_Load           (MCCdevPtr dev);
void das6036_Init           (void);
...
int das6036_ControlCallback (int panel, int control, int event, void *callbackData, int eventData1, int eventData2)
{
    ...
    if (event == EVENT_COMMIT  && port)
        das6036_UpdateMeasurePanel(port);
    return 0;
}
    ...
    switch (menuItem)
    {
        ...
        case DASMENU_MEAS_IN_7:
            if(utilG.acq.status != ACQ_BUSY){
                ...
                MCCdevPtr dev = port->port.analogueIOport.IO.acqchan->dev;
                das6036Ptr das = dev->device;
                port->measPanel = port->measPanel? port->measPanel : LoadPanel(utilG.p, "DAS-6036u.uir", MEASURE);
        ...
        case DASMENU_FILE_SAVE:
            boards_Save(callbackData, das6036_Save);
            break;
        case DASMENU_FILE_LOAD:
            boards_Load(callbackData, das6036_Load);
            break;
    }
}
...
int  das6036_IndexFromRange (int val)
{
    ...
    switch(val)
    {
        ...
        case BIPPT5VOLTS:   range = 2;break;
        case BIPPT05VOLTS:  range = 3;break;
    }
    return range;
}
...
void das6036Operate (int menubar, int menuItem, void *callbackData, int panel)
{
    ...
    if(!das->panel)
    {
        das->panel = LoadPanel(utilG.p, "DAS-6036u.uir", DAS_CTRL);
        das->menu = LoadMenuBar(das->panel, "DAS-6036u.uir", DASMENU);
    ...
    devPanel_Add (das->panel, dev, das6036_UpdateReadings);
...
void das6036_Init(void)
{
    ...
    {
        util_ChangeInitMessage ("pci-das6036...");
        if(devType)
        {
            Fmt(devType->id, DAS6036_ID);
            Fmt(devType->label, "pci das6036");
            devType->CreateDevice   = das6036_Create;
            devType->RemoveDevice   = das6036_Remove;
            devType->OperateDevice  = das6036Operate;
            devType->UpdateReadings = das6036_UpdateReadings;
